...
import { ExpenseResponse, IApiResponse, ApproveExpenseRequest, DeclineExpenseRequest } from 'app/_models';

@Component({
  selector: 'app-process-expense',
  templateUrl: './process-expense.component.html',
  styleUrls: ['./process-expense.component.scss']
})
export class ProcessExpenseComponent implements OnInit {
  ...
  expenseId: number;
  managerId: number;
  myExpenseDetails: ExpenseResponse;
  projectvalue: string;
  reasonvalue: string;
  ...
  constructor(public alertService: AlertService,
    ...
    private spinner: NgxSpinnerService) {
      this.myExpenseDetails = new ExpenseResponse();
  ...
  private loadData() {
    return this.dataService.processExpenseById(this.expenseId)
        .subscribe(tsInfo => {          
          this.myExpenseDetails = tsInfo as ExpenseResponse;
          if (this.myExpenseDetails !== null) {
            this.expenseForm.get("weekEnding").patchValue(this.myExpenseDetails.weekEnding);
            this.expenseForm.get("projectName").patchValue(this.myExpenseDetails.projectName);
            this.expenseForm.disable();
            this.expenseForm.controls.approverName.enable();
            this.expenseForm.controls.approverName.patchValue('');
          }
          window.scrollTo(0, 0);
          this.spinner.hide();
        },
    ...
    setMileage(event: any, day: any) {  
      switch (day) {
        case 'sun':
          ...
          this.myExpenseDetails.sundayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('sun');          
          this.updateWeeklyExpenses();
          break;
        case 'mon':
          this.myExpenseDetails.mondayMiles = +event.target.value;
          this.myExpenseDetails.mondayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('mon');
          this.updateWeeklyExpenses();
          break;
        case 'tues':
          this.myExpenseDetails.tuesdayMiles = +event.target.value;
          this.myExpenseDetails.tuesdayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('tues');
          this.updateWeeklyExpenses();
          break;
        case 'wed':
          this.myExpenseDetails.wednesdayMiles = +event.target.value;
          this.myExpenseDetails.wednesdayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('wed');
          this.updateWeeklyExpenses();
          break;
        case 'thur':
          this.myExpenseDetails.thursdayMiles = +event.target.value;
          this.myExpenseDetails.thursdayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('thur');
          this.updateWeeklyExpenses();
          break;
        case 'fri':
          this.myExpenseDetails.fridayMiles = +event.target.value;
          this.myExpenseDetails.fridayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
          this.updateDailyExpenses('fri');
          this.updateWeeklyExpenses();
          break;
        case 'sat':
          this.myExpenseDetails.saturdayMiles = +event.target.value;
          this.myExpenseDetails.saturdayMileage = +event.target.value * this.myExpenseDetails.mileageRate;
  ...
  public submitExpense = (action) => {
    if (this.expenseForm.valid) {
      this.spinner.show();
      switch(action) {
        case 'decline':
          if (this.expenseForm.controls.rejectReason.value === undefined || this.expenseForm.controls.rejectReason.value === '') {
            this.expenseForm.controls.rejectReason.setErrors({required: true});
            this.spinner.hide();
            return false;
          }
          const declineReq: DeclineExpenseRequest = {
            expenseId: +this.expenseId,
            approverId: +this.managerId,
            contractorId: +this.myExpenseDetails.contractorId,
            contractorName: this.myExpenseDetails.contractorName,
            weekEnding: this.expenseForm.controls.weekEnding.value,
            reason: this.expenseForm.controls.rejectReason.value,
            approverName: this.expenseForm.controls.approverName.value,
            approveTime: this.datePipe.transform(new Date(), 'MM/dd/yyyy h:mm a')
          };
          this.executeDeclineExpense(declineReq);
          break;
        case 'approve':
          const apprReq: ApproveExpenseRequest = {
            expenseId: +this.expenseId,
            approverId: +this.managerId,
            approverName: this.expenseForm.controls.approverName.value,
            approveTime: this.datePipe.transform(new Date(), 'MM/dd/yyyy h:mm a')
          };
          this.executeApproveExpense(apprReq);
          break;
        case 'approve with edit':
          this.executeEditApproveExpense();
          break;        
      }
  ...
  private executeDeclineExpense = (request) => {
    this.dataService.declineExpense(request)
      .pipe(first())
  ...
  private executeApproveExpense = (request) => {
    this.dataService.approveExpense(request)
        .pipe(first())
        .subscribe((response: IApiResponse) => {
          this.alertService.success(response.message);
          window.scrollTo(0, 0);
  ...
  private executeEditApproveExpense = () => {
    const request = this.setExpenseRequest() as FormData;
    this.dataService.editApproveExpense(request)
        .pipe(first())
        .subscribe((response: IApiResponse) => {
          this.alertService.success(response.message);
          window.scrollTo(0, 0);
  ...
  private setExpenseRequest(): FormData  {
    ...
    formData.append('expenseId', this.myExpenseDetails.expenseId.toString());
    formData.append('weekEnding', this.expenseForm.controls.weekEnding.value);
    formData.append('projectName', this.expenseForm.controls.projectName.value);
    formData.append('approverId', this.managerId.toString());
    formData.append('approverName', this.expenseForm.controls.approverName.value);
    formData.append('approveTime', this.datePipe.transform(new Date(), 'MM/dd/yyyy h:mm a'));
    formData.append('prepaidExpense', this.myExpenseDetails.prePaidExpense.toString());
    formData.append('mondayDate', this.myExpenseDetails.mondayDate.toString());
    formData.append('tuesdayDate', this.myExpenseDetails.tuesdayDate.toString());
    formData.append('wednesdayDate', this.myExpenseDetails.wednesdayDate.toString());
    formData.append('thursdayDate', this.myExpenseDetails.thursdayDate.toString());
    formData.append('fridayDate', this.myExpenseDetails.fridayDate.toString());
    formData.append('saturdayDate', this.myExpenseDetails.saturdayDate.toString());
    formData.append('sundayDate', this.myExpenseDetails.sundayDate.toString());
  ...
  addEvent(event: MatDatepickerInputEvent<Date>) {
    if (event.value !== this.myExpenseDetails.weekEnding) {
        ...
        this.spinner.show();
        this.dataService.expenseExists(this.myExpenseDetails.assignmentId, this.datePipe.transform(this.myExpenseDetails.weekEnding, 'yyyy-MM-dd'))
  ...
  getApproverNameErrorMessage() {
    if (this.expenseForm.controls.approverName.hasError('required')) {
      return 'Please enter approver\'s name';
    }
  }

  getRejectReasonErrorMessage() {
    if (this.expenseForm.controls.rejectReason.hasError('required')) {
...
openDeclineDialog(declineDialog) {
  ...
  enableExpenseForm() {
    ...
    this.expenseForm.controls.weekEnding.disable();
    return false;
  }
  declineExpense() {
  ...
  navigateToProcessExpense() {
    this.router.navigate(['process-expense'], { queryParams: { 'expenseid': this.expenseId,'managerid': this.managerId } });
