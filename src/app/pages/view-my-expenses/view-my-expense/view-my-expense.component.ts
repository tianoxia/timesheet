...
import { ExpenseResponse, ExpenseRequest, IApiResponse } from 'app/_models';

@Component({
  selector: 'app-view-my-expense',
  templateUrl: './view-my-expense.component.html',
  styleUrls: ['./view-my-expense.component.scss']
})
export class ViewMyExpenseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.isRejected = this.route.snapshot.queryParams['expensestatus'] === 'Rejected';
    this.releaseExpense = this.route.snapshot.queryParams['releaseexpense'] === 'true';
    this.spinner.show();
    this.loadData();
  }

  private loadData() {
    return this.dataService.getExpenseById(this.expenseId)
        .subscribe(tsInfo => {
          ...
          this.expenseForm.controls.isReleased.patchValue(this.myExpenseDetails.isReleased);
          this.isReleased = this.myExpenseDetails.isReleased ? 'Yes' : 'No';
          this.eligibleToRelease = this.myExpenseDetails.expenseStatus !== 'Pending' && this.myExpenseDetails.expenseStatus !== 'Rejected';
          this.expenseForm.controls.isReleased.valueChanges.subscribe((value) => {
            this.isReleased = value ? 'Yes' : 'No';
          });
          if (this.eligibleToRelease) {
            this.expenseForm.disable();
            if (this.releaseExpense) {
              this.expenseForm.controls.isReleased.enable();
            }
          }
          window.scrollTo(0, 0);
          this.spinner.hide();
  ...
  public submitExpense = () => {
    const details = this.myExpenseDetails;
    if (this.expenseForm.valid) {
      this.spinner.show();
      this.executeUpdateExpense();
    }
  }
  ...
  private setExpenseRequest(): FormData  {
    ...
    formData.append('assignmentId', this.myExpenseDetails.assignmentId.toString());
    formData.append('contractorName', this.myExpenseDetails.contractorName);
    formData.append('contractorId', this.myExpenseDetails.contractorId.toString());
  ...
  navigateToPreviousPage() {
    if (this.releaseExpense) {
      this.router.navigate(['release-expenses'], { queryParams: {type:'expense'} });
    } else {
      this.router.navigate(['view-my-expenses'], { queryParams: {type:'expense'} });
    } 
  }
}
